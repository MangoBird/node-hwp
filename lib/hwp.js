...
var err = function (f, s) {
  f(new Error(s));
};
var todo = function (f, s) {
  err(f, s ? "TODO: " + s : "TODO");
};
...
var getPropSet = function (stream, callback) {
  getBuffer(stream, function (e, buffer) {
    ...
    var set = {},
      numProps = buffer.readUInt32LE(0x34);
    ...
    for (var i = 0; i < numProps; i++) {
      ...
      switch (type) {
        ...
        case 18: // VT_UI2
          ...
          break;
        case 3:
        case 10: // VT_I4, VT_ERROR
        case 19: // VT_UI4
          value = buffer.readUInt32LE(offset + 4);
          break;
        case 8:
        case 30: // VT_BSTR, VT_LPSTR
...
HWP.prototype.loadFromHWP = function (file, callback, option) {
  ...
  if (this._loaded || this._loading) {
    err(callback, "HWP document is alreadly loaded!");
    return;
  }
  this._loading = true;
    ...
    getBuffer(_this._doc.stream("FileHeader"), function (e, buffer) {
      ...
      if (meta.encrypted || meta.distributed) {
        todo(
          _cb,
          "Can't open " +
            (meta.encrypted ? "encrypted" : "distributed") +
            " HWP file."
        );
        return;
      }
...
HWP.prototype._setHead = function (callback, option) {
  if (!this._hml || this._hml.findChild("HEAD")) {
    err(callback, "Invalid _setHead invocation!");
    return;
  }
      ...
      try {
        ...
        docInfoTree.forEach(function (record) {
          ...
          switch (record.name) {
              ...
              record.children.forEach(function (r) {
                switch (r.name) {
                  ...
                  case "FACE_NAME":
                    ...
                    var fontface = list.getChildWith(
                      "FONTFACE",
                      "Lang",
                      HEnum.LangType[fontKind]
                    );
                    elem = new HNode.FONT();
                  ...
                  case "CHAR_SHAPE":
                    ...
                    elem.setAttr(
                      r.attr,
                      "Height TextColor ShadeColor UseFontSpace UseKerning SymMark BorderFillId".split(
                        " "
                      )
                    );

                    for (k = 0; k < 7; k++) {
                      elem.getChild("FONTID").attr[HEnum.LangType[k]] =
                        r.attr._FontIDs[k].value;
                      elem.getChild("RATIO").attr[HEnum.LangType[k]] =
                        r.attr._FontRatios[k].value;
                      elem.getChild("CHARSPACING").attr[HEnum.LangType[k]] =
                        r.attr._FontCharSpacings[k].value;
                      elem.getChild("RELSIZE").attr[HEnum.LangType[k]] =
                        r.attr._FontRelSizes[k].value;
                      elem.getChild("CHAROFFSET").attr[HEnum.LangType[k]] =
                        r.attr._FontCharOffsets[k].value;
                    }
                  ...
                  default:
                    console.warn(
                      "Warning [head>mapping]: not processing record %s",
                      r.name
                    );
            ...
            default:
              console.warn(
                "Warning [head]: not processing record %s",
                record.name
              );
...
HWP.prototype._setBody = function (callback, option) {
  if (!this._hml || this._hml.findChild("BODY")) {
    err(callback, "Invalid _setBody invocation!");
    return;
  }
...
// Reads sections
HWP.prototype._setSections = function (
  body,
  storage,
  sid,
  numSections,
...
HWP.prototype._getHML = function (parent, record) {
  ...
  switch (record.name) {
    case "PARA_HEADER":
      ...
      node.setAttr(
        record.attr,
        "ParaShape Style PageBreak ColumnBreak InstId".split(" ")
      );
      // 0이면 생략되는 듯?
      ...
      for (i = 0; i < record.children.length; i++) {
        ...
        id = ch.name;
        if (id == "CTRL_HEADER") {
          if (!controlTable[ch.attr._Type]) controlTable[ch.attr._Type] = [];
          ...
          if (!recordTable[id]) recordTable[id] = [];
      ...
      var insertRange = function insertRange(elem, start, end) {
        while (range_i < rangeArr.length) {
          ...
          switch (r[1]) {
            ...
            default:
              console.warn(
                "Warning [text]: not processing range tag %d",
                rangeArr[range_i][1]
              );
          }
          if (rangeTag) {
      ...
      if (shape.length == 0 || shape[0].start != 0) {
        throw new Error(
          "PARA_CHAR_SHAPE must have a CHAR_SHAPE starts from 0!"
        );
      }
      for (i = 0; i < text.length; i++) {
        next =
          i + 1 < text.length
            ? text[i + 1].start
            : recordTable["PARA_TEXT"][0].data.length / 2;
        // 지나친 게 있는지 확인
        while (
          shapeInd < shape.length &&
          shape[shapeInd].start < text[i].start
        ) {
        ...
        switch (text[i].type) {
          case "text":
            ...
            while (shapeInd < shape.length && shape[shapeInd].start < next) {
              ...
              elem.value = String.fromCharCode.apply(
                String,
                text[i].data.slice(si, ei)
              );
              // Range Tag 삽입
          ...
          case 23: // Dutmal
            ...
            if (
              !controlTable[cid] ||
              controlTable[cid].length <= controlIndex[cid]
            ) {
            ...
            celem = this._getHML(
              textElem,
              controlTable[cid][controlIndex[cid]++]
            );
          ...
          default:
            console.warn(
              "Warning [text]: not processing text %s",
              text[i].type
            );
        }

        // 컨트롤 문자에 의한 오프셋을 더한다.
        if (text[i].type != "text" && "data" in text[i])
          control_offset += text[i].data.length + 2;
      }
    ...
    case "FOOTNOTE_SHAPE":
      ...
      record.attr.NotePlacement.Place = HEnum.get(
        (isEnd ? "End" : "Foot") + "NoteShapePlaceType",
        record.attr.NotePlacement.Place
      );
    ...
    case "CTRL_HEADER":
      ...
      if (record.attr.ShapeObject) {
        ...
        shapeObject
          .getChild("POSITION")
          .setAttr(record.attr.ShapeObject.Position);
        shapeObject
          .getChild("OUTSIDEMARGIN")
          .setAttr(record.attr.ShapeObject.OutsideMargin);
        if (record.attr.ShapeObject.ShapeComment.value)
          shapeObject.getChild("SHAPECOMMENT").value =
            record.attr.ShapeObject.ShapeComment.value;
      }
      switch (record.attr._Type) {
        ...
        case "tbl ": // TABLE
          ...
          if (r_table.name != "TABLE") {
            console.warn(
              "Warning [body>control>tbl]: expected TABLE, got %s",
              r_table.name
            );
            break;
          }
          ...
          for (i = 0; i < r_table.attr._RowSizes.length; i++) {
            ...
            for (j = 0; j < row_len; j++) {
              ...
              if (r_list.name != "LIST_HEADER") {
                console.warn(
                  "Warning [body>control>tbl>list]: expected LIST_HEADER, got %s",
                  r_list.name
                );
                break;
              }
              ...
              for (k = 0; k < r_list.attr._ParaCount; k++) {
                ...
                if (r_para.name != "PARA_HEADER") {
                  console.warn(
                    "Warning [body>control>tbl>list]: expected PARA_HEADER, got %s",
                    r_para.name
                  );
                  break;
                }
        ...
        default:
          console.warn(
            "Warning [body>control]: not processing control header '%s'",
            record.attr._Type
          );
          debug_warn(record.toString());
      }
    ...
    case "SHAPE_COMPONENT":
      var drawingObject = null,
        i;
      var shapeComponent = new HNode.SHAPECOMPONENT(),
        renderingInfo;
      ...
      renderingInfo
        .getChild("TRANSMATRIX")
        .setAttr(record.attr.RenderingInfo.TransMatrix);
      record.attr.RenderingInfo._ScaRotMatrices.forEach(function (elem) {
        var scaMatrix = new HNode.SCAMATRIX(),
          rotMatrix = new HNode.ROTMATRIX();
        scaMatrix.setAttr(elem.ScaMatrix);
        rotMatrix.setAttr(elem.RotMatrix);
        renderingInfo.add(scaMatrix);
        renderingInfo.add(rotMatrix);
      });
      // Process children of each SHAPE_COMPONENTs
      var procShapeRecord = function procShapeRecord(tag, func) {
        if (!func)
          func = function simpleAttrSet(elem) {
        ...
        record.children.forEach(function (elem) {
          switch (elem.name) {
            case "LIST_HEADER":
              var textMargin,
                r_drawtext = new HRecord.LIST_DRAWTEXT(
                  elem.data.slice(elem.offset)
                );
            ...
            case "PARA_HEADER":
              if (paraList == null) {
                console.warn(
                  "Warning [%s>SHAPE_COMPONENT]: PARA_HEADER without LIST_HEADER",
                  parent.name
                );
                break;
              }
            ...
            case "SHAPE_COMPONENT":
              ...
              if (node.name != "CONTAINER") {
                console.warn(
                  "Warning [%s>SHAPE_COMPONENT]: SHAPE_COMPONENT in %s",
                  parent.name,
                  node.name
                );
              }
            ...
            default:
              ...
              console.warn(
                "Warning [%s>SHAPE_COMPONENT]: not processing child record %s",
                parent.name,
                elem.name
              );
      ...
      switch (record.attr._Type) {
        ...
        default:
          console.warn(
            "Warning [%s>SHAPE_COMPONENT]: not processing shape type %s",
            parent.name,
            record.attr._Type
          );
          debug_warn(record.toString());
    ...
    default:
      console.warn(
        "Warning [%s]: not processing record %s",
        parent.name,
        record.name
      );
      debug_warn(record.toString());
